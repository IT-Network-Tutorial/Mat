...
int ir_ramchap;
int ir_chap;
int ir_hlram;//Promìnné pro závory
...
int main()
{
	...
	{
		stav_zavor = inportb(0x301);//Naètení do promìnné
		...
		else
		{
			...
			port1 = port1 & maska_smer0;//Otáèení vlevo
	...
	while (zavora_hlrameno != 1)//Dokud není závora aktivní
	{
		...
		else
		{
			...
			port1 = port1 & maska_smer0;//smìr nahoru
	...
	while (zavora_chapadlo != 1)//Dokud není závora aktivní
	{
		...
		else
		{
			...
			port1 = port1 & maska_smer0;//otevøít chapadlo
	...
	while (zavora_ramenochap != 1)//Dokud není závora aktivní
	{
		...
		else
		{
			...
			port1 = port1 | maska_smer1;//smìr nahoru
	...
	while (1)//Nekoneèý cyklus
	{
		...
		stav_zavor = inportb(0x301);//Naètení do promìnn
		ir_ot = stav_zavor | maska_otaceni; //Masky mohu využít stejné
		if (ir_ot == 0x7f)//Pokud je na daném bitu 0
		{
			printf("Dosazeno mezni polohy, stisk a zakazan.");//Indikace v pc
			zavora_otaceni = 1;//Koncák je aktivní
			t_ot = 0;//nulování poèítání taktù
		}
		...
		if (ir_hlram == 0xbf)//Pokud je na daném bitu 0
		{
			...
			zavora_hlrameno = 1;//Koncák je aktivní
			t_hlram = 0;//nulování poèítání taktù
		}
		...
		if (ir_chap == 0xdf)//Pokud je na daném bitu 0
		{
			...
			zavora_chapadlo = 1;//Koncák je aktivní
			t_chap = 0;//nulování poèítání taktù
		}
		...
		if (ir_ramchap == 0xef)//Pokud je na daném bitu 0
		{
			...
			zavora_ramenochap = 1;//Koncák je aktivní
			t_ramchap = 0;//nulování poèítání taktù
		}
		else zavora_ramenochap = 0;//Pokud ne, není aktivní

		if (kbhit())//detekce stisknutého tlaèítka
		{
			klavesa = getch(); //jeho zapsání do promìnné
			...
			case 97://a
				...
				break;
			case 100://d
				if (t_ot < 1800) //Pokud je poèet taktù menší než 1800
				{
					port2 = port2 & maska_otaceni;
					port1 = port1 | maska_smer1;//smìr vpravo
					t_ot++;//Pøièítání taktù
				}
				else
				{
			...
			case 119://w
				if (zavora_hlrameno != 1)//Pokud závora není aktivní
				{
					port2 = port2 & maska_hlrameno;//aktivace hl. ramena
					port1 = port1 & maska_smer0;//smìr nahoru
					t_hlram--;//Odeèítání taktù
				}
				break;
			case 115://s
				if (t_hlram < 500)//Pokud je poèet taktù menší než 500
				{
					port2 = port2 & maska_hlrameno;//aktivace hl. ramena
					port1 = port1 | maska_smer1;//smìr dolù
					t_hlram++;//Pøièítání taktù
				}
				else
				{
			...
			case 102://f
				if (zavora_chapadlo != 1)//Pokud závora není aktivní
				{
					port2 = port2 & maska_chapadlo;//aktivace chapadla
					port1 = port1 & maska_smer0;//otevøít chapadlo
					t_chap--;//Odeèítání taktù
				}
				break;
			case 103://g
				if (t_chap < 450)//Pokud je poèet taktù menší než 450
				{
					port2 = port2 & maska_chapadlo;//aktivace chapadla
					port1 = port1 | maska_smer1;//zavøít chapadlo
					t_chap++;//Pøièítání taktù
				}
				else
				{
			...
			case 109://m
				if (zavora_ramenochap != 1)
				{
					port2 = port2 & maska_ramenochapadlo;//aktivace pohybu chapadla
					port1 = port1 | maska_smer1;//smìr nahoru
					t_ramchap--;//Odeèítání taktù
				}
				break;

			case 110://n
				if (t_ramchap < 3000)//Pokud je poèet taktù menší než 3000
				{
					port2 = port2 & maska_ramenochapadlo;//aktivace pohybu chapadla
					port1 = port1 & maska_smer0;//smìr dolu
					t_ramchap++;//Pøièítání taktù
				}
				else
				{
		...
		else //Pokud není stisknuté tlaèítko 
		{
			port1 = 0xff;
